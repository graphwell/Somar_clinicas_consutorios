...
const getTimesForRange = (start: string = "08:00", end: string = "18:00") => {
  const times = [];
...
export default function AgendaPage() {
  ...
  // Horários Dinâmicos baseados na Clínica
  const HOURS = useMemo(() => {
    return getTimesForRange(clinica?.openingTime, clinica?.closingTime);
  }, [clinica]);
      ...
      <div className="animate-premium mt-8">
        ...
        {activeTab === 'profissionais' && (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {profissionais.map(p => {
              // Verifica se o profissional trabalha hoje
              const dayOfWeek = selectedDate.getDay();
              const escalaHoje = p.escalas?.find(e => e.diaSemana === dayOfWeek && e.ativo);
              
              return (
                <div key={p.id} className="bg-white border border-card-border p-8 rounded-[2.5rem] shadow-premium space-y-8 hover:scale-[1.01] transition-transform group">
                  <div className="flex items-center gap-5 border-b border-slate-50 pb-6">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h4 className="text-base font-black text-text-main uppercase italic leading-tight tracking-tighter">{p.nome}</h4>
                        <div className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase ${escalaHoje ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                          {escalaHoje ? `${escalaHoje.horaInicio} - ${escalaHoje.horaFim}` : 'Folga'}
                        </div>
                      </div>
                  ...
                  <div className="space-y-3">
                    {/* Grade de 10 min RESUMIDA para o profissional */}
                    <div className="grid grid-cols-4 gap-1.5 opacity-80">
                      {getTimesForRange(escalaHoje?.horaInicio || "08:00", escalaHoje?.horaFim || "12:00").filter((_, i) => i % 3 === 0).map(h => {
                         const appt = appointments.find(a => isSameDay(new Date(a.dataHora), selectedDate) && a.profissional?.id === p.id && formatTime(a.dataHora) === h);
                         return (
                           <div key={h} className={`py-2 rounded-lg text-center text-[8px] font-black border ${appt ? 'bg-primary text-white border-primary' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                             {h}
                           </div>
                         );
                      })}
                    ...
                    {appointments.filter(a => isSameDay(new Date(a.dataHora), selectedDate) && a.profissional?.id === p.id).length === 0 && (
                      <p className="py-6 text-center text-[9px] font-black text-text-placeholder uppercase tracking-[0.3em] bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">Sem agendamentos</p>
                    )}
                  </div>
                </div>
              );
        ...
        {/* VIEW: SERVIÇOS (LISTAGEM DE OPÇÕES) */}
        {activeTab === 'servicos' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map(s => (
              <div key={s.id} className="bg-white border border-card-border p-8 rounded-[2.5rem] shadow-premium space-y-6 hover:shadow-2xl transition-all group">
                <div className="flex justify-between items-start">
                  <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-xl group-hover:bg-primary/10 group-hover:text-primary transition-colors">💼</div>
                  <span className="text-sm font-black text-primary">R$ {s.preco}</span>
                </div>
                <div>
                  <h4 className="text-lg font-black text-text-main uppercase italic tracking-tighter leading-none">{s.nome}</h4>
                  <div className="flex gap-4 mt-3">
                    <span className="text-[10px] font-black text-text-placeholder uppercase tracking-widest">⏱️ {s.duracaoMinutos} MIN</span>
                    <span className="text-[10px] font-black text-text-placeholder uppercase tracking-widest">🛡️ {s.bufferTimeMinutes} MIN BUFFER</span>
                  </div>
                </div>
                <button 
                  onClick={() => setShowModal(true)}
                  className="w-full py-4 rounded-2xl bg-slate-50 text-text-main text-[10px] font-black uppercase tracking-widest group-hover:bg-primary group-hover:text-white transition-all shadow-sm"
                >
